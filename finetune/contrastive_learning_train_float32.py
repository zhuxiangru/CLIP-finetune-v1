# 用数据增强得到的十倍图片finetune,对比test效果（在雪瑶的数据集上,摘取出人物概念，其他概念不要）：
# 0.原始vit,不finetune                  acc=13.14%
# 1.原始vit,不用google图片finetune,用雪瑶数据集finetune,取20%再次finetune   acc=14.47%
# 2.用原图片finetune（file1, file2）     只用google图片finetuen acc=6.93% , 用google图片finetune后，再用雪瑶数据集finetune,取20%再次finetune  acc=12.39%
# 3.用原图片聚类后的图片对finetune（要求相似图片对必须在一个batch里）（file1, file2）
# 4.用原图片增强，再通过rule筛选后的图片对finetune（要求相似图片对必须在一个batch里） （file1, file2, modified_file1, modified_file2）
# 5.用原图片增强，再通过分类器筛选后的图片对finetune (file1, file2, selected_file1, selected_file2)     选图score>0.1  acc=6.93%, 用google图片finetune后，再用雪瑶数据集finetune,取20%再次finetune  acc=12.21%
# 选图score>0.5  acc=7.04%, 用google图片finetune之后，再用雪瑶数据集finetune, 取20%finetune， acc=12.92%
# 选图score>0.8  acc=7.60%, 用google图片finetune之后，再用雪瑶数据集finetune, 取20%finetune， acc=14.45%
# 6. 用原图片增强，再通过分类器筛选后的图片对和雪瑶20%数据集finetune (file1, file2, selected_file1, selected_file2, validate_file)   选图score>0.8, acc=13.37%
...
# 根据爬取的图片，进一步数据增强的图片
def load_modified_data(input_json_file):
    data_list = []
    label_list = []
    with open(input_json_file, "r", encoding="utf-8") as infile:
        ...
        result_json = json.loads(lines)
        for key, value in result_json.items():
            label_list.append(key)
            for value_ele in value:
                #if value_ele[1] > 0.1:
                if value_ele[1] > 0.8:
                    data_list.append([value_ele[0], key])
...
def load_data(parent_dir_name, config):    
    ...
    return train_data_list, list(set(train_labels_list)), validate_data_list, list(set(validate_labels_list))
...
def load_data_general_v2(parent_dir_name, config):
    images_list = []
    labels_list = []
    data_list = []
    for dirname, _, filenames in os.walk(parent_dir_name):
    ...
    return data_list, list(set(labels_list))
...
def main_v1():
    ...
    parent_dir_name = "/home/featurize/work/contrastive_leanring/image_clustering/data/person_images_photo_test_20"
...
def main_v2():
    ...
    # 原图
    parent_dir_name = "/home/featurize/work/contrastive_leanring/image_clustering/data/person_images_photo_test_20"
    data_list, label_list = load_data_general_v2(parent_dir_name, config)  # [[image_tensor, label_tensor], ....]
    # 数据增强后的图
    input_modified_json_file = "/home/featurize/work/vision-transformer-pytorch/experiments/save/ft_ImageList_bs32_lr0.03_wd0.0_210604_203738/results/predict_label_output.json"
    modified_data_list, modified_label_list = load_modified_data(input_json_file=input_modified_json_file)
    ...
    parent_dir_name = "evaluation/validates/"
    xueyao_data_list, xueyao_label_list, xueyao_validate_data_list, xueyao_validate_label_list = load_data(parent_dir_name, config)  # [[image_tensor, label_tensor], ....]
    
    data_list.extend(modified_data_list)
    label_list.extend(modified_label_list)
    data_list.extend(xueyao_data_list)
    label_list.extend(xueyao_label_list)
